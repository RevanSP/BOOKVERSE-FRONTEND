...
import AlertManga from "../components/AlertManga";
...
const MangaSection: React.FC = () => {
    ...
    const genres = Array.from(new Set(mangaData.flatMap(manga => manga.info.genres)));
    ...
    const mangaTotal = mangaData.filter(manga =>
    ...
    useEffect(() => {
        const storedHistory = localStorage.getItem('mangaReadingHistory');
        if (storedHistory) {
    ...
    const handleChapterClick = (chapter: Chapter) => {
        ...
        setReadingHistory((prevHistory) => {
            const newHistory = {
                ...prevHistory,
                [selectedManga?.title || ""]: chapter.number as unknown as number,
            };
    ...
    const handleCardClick = (manga: Manga) => {
        ...
        const modal = document.getElementById('mangaModal') as HTMLDialogElement;
        if (modal) {
            modal.showModal();
        }
    };
    ...
    const filteredMangaData = mangaData.filter(manga => {
        const matchesGenre = selectedGenre ? manga.info.genres.includes(selectedGenre) : true;
        const matchesSearch = manga.title.toLowerCase().includes(searchQuery.toLowerCase());
        return matchesGenre && matchesSearch;
    });

    const totalPages = Math.ceil(filteredMangaData.length / itemsPerPage);

    const handlePageChange = (page: number) => {
        setCurrentPage(page);
    };

    const currentMangaData = filteredMangaData.slice(
        (currentPage - 1) * itemsPerPage,
        currentPage * itemsPerPage
    );

    const hasResults = currentMangaData.length > 0;
    ...
    const genreCounts = genres.map((genre) => ({
        name: genre,
        count: mangaData.filter(manga => manga.info.genres.includes(genre)).length,
    }));
    ...
    return (
        <>
            <Layout>
                <div className="flex flex-col sm:flex-row justify-between items-center mb-8 mt-2">
                    <div className="flex justify-between items-center w-full">
                        <h1 className="text-4xl sm:text-5xl font-bold" id="MANGA">MANGA</h1>
                        <div className="dropdown dropdown-bottom dropdown-end sm:hidden">
                            ...
                            <ul
                                tabIndex={0}
                                className="dropdown-content menu menu-xs bg-base-100 rounded-box z-[1] w-72 p-2 shadow border-2 border-neutral-content mt-1 overflow-y-auto max-h-96"
                            >
                ...
                <div className="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4" data-aos="fade-up">
                    {hasResults ? (
                        currentMangaData.map((manga, index) => {
                            return (
                                <div
                                    key={index}
                                ...
                                >
                                    <img loading="lazy"
                                        ...
                                        referrerPolicy="no-referrer"
                                    />
                                    <p className="text-md text-center truncate font-bold mt-2 px-3">{manga.title}</p>
                                    <div className="rating rating-xs flex justify-center mt-1 mb-3">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <input
                                                key={star}
                                                type="radio"
                                                name={`rating-${manga.title}`}
                                                className="mask mask-star-2 bg-orange-400"
                                                checked={Math.round(parseFloat(manga.rating) / 2) === star}
                                                readOnly
                                            />
                                        ))}
                                    </div>
            ...
            <dialog ref={modalRef} id="mangaModal" className="modal">
                <div className="modal-box w-11/12 max-w-6xl border-2 border-neutral-content rounded">
                    ...
                    {selectedManga && (
                        <>
                            <h3 className="font-bold text-lg mb-4">MANGA</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="p-4 bg-base-200 rounded flex border-2 border-neutral-content relative">
                                    ...
                                    <ul className="ml-4 list-disc list-inside space-y-1">
                                        <li className="text-sm"><strong>Title :</strong> {selectedManga.title}</li>
                                        <li className="text-sm"><strong>Author :</strong> {selectedManga.info.author}</li>
                                        <li className="text-sm"><strong>Genres :</strong> {selectedManga.info.genres.join(", ")}</li>
                                        <li className="text-sm">
                                            <strong>Recent Read :</strong> {readingHistory[selectedManga?.title] !== undefined ? `Chapter ${readingHistory[selectedManga.title]}` : "NONE"}
                                        </li>
                                    </ul>
